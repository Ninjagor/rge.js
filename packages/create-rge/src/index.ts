...
const createMultifileProject = (projectName: string) => {
    ...
    const rgeGradient = gradient(Object.values(poimandresTheme));
...
`
        ...
        const gameJsPath = path.join(scriptsDir, 'game.js');
        fs.writeFileSync(
            gameJsPath,
            `
const r = rgejs;
...
`
        ...
        const configJsPath = path.join(scriptsDir, 'configs.js');
        fs.writeFileSync(
            configJsPath,
            `
...
`
        ...
        const storeJsPath = path.join(scriptsDir, 'store.js');
        fs.writeFileSync(
            storeJsPath,
            `
const variableStore = {
...
`
        ...
        const engineJsPath = path.join(scriptsDir, 'engine.js');
        fs.writeFileSync(
            engineJsPath,
            `
...
`
        ...
        fs.mkdirSync('RuntimeFunctions')
        const RFdir = path.join(scriptsDir, 'RuntimeFunctions');
        process.chdir(RFdir);
        fs.mkdirSync('preload')
        fs.mkdirSync('setup')
        fs.mkdirSync('tick')
        const indexJsOne = path.join(RFdir, 'index.js');
        fs.writeFileSync(
            indexJsOne,
            `
...
`
        ...
        const preloadDir = path.join(RFdir, 'preload');
        process.chdir(preloadDir);
        const indexJsTwo = path.join(preloadDir, 'index.js');
        fs.writeFileSync(
            indexJsTwo,
            `
...
`
        ...
        const setupDir = path.join(RFdir, 'setup');
        process.chdir(setupDir);
        const indexJsThree = path.join(setupDir, 'index.js');
        fs.writeFileSync(
            indexJsThree,
            `
...
`
        ...
        const tickDir = path.join(RFdir, 'tick');
        process.chdir(tickDir);
        const indexJsFour = path.join(tickDir, 'index.js');
        fs.writeFileSync(
            indexJsFour,
            `
